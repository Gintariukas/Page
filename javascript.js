var dateObj = new Date,
    ...
    dayCountPerMonth = (new Date(year, month + 1, 1, -1)).getDate(),
    today = date + "-" + (month + 1) + "-" + year,
    template = [],
    startDate;
console.log(date);
for (i = startDate = 3 < date ? date - 3 : 1; i < startDate + 7; i++) template.push(i);

function updateNewYear() { year += 1 }

function updateMonth() { 12 == month + 1 && (month = 0) }
var fakeTemplate = 0,
    c = 0,
    fakeMonth = 0,
    activeClass = "";
for (i = 0; i < template.length; i++) {
    template[i] > dayCountPerMonth && (fakeTemplate += 1, template[i] = fakeTemplate, 0 == c && (updateNewYear(), updateMonth(), c = 1));
    var loopDate = year + "-" + (month + 1) + "-" + template[i],
        loopDate = new Date(loopDate),
        loopDate = loopDate.getDay();
    template[i] + "-" + (month + 1) + "-" + year == today && (activeClass = "active");
    $("body table tr").append('<td class="' + activeClass + '" data-date="' + year + "-" + (month + 1) + "-" + template[i] + '" data-day="' + dayNames[loopDate] + '"><span>' + dayNames[loopDate] + "</span><span>" + template[i] +
        "</span></td>");
    activeClass = ""
}
...
function DateTime() {
    ...
    this.day = {
        ...
        of: {
            ...
            (this.date.getDay() < 10
                ? "0" + this.date.getDay()
                : this.date.getDay()) + getDaySuffix(this.date.getDay()),
            month:
            (this.date.getDate() < 10
                ? "0" + this.date.getDate()
                : this.date.getDate()) + getDaySuffix(this.date.getDate())
        }
    };

    this.month = {
        index: this.date.getMonth() + 1 < 10
    ...
    ko.bindingHandlers.daterangepicker = (function() {
        return $.extend(this, {
            ...
            init: function(element, valueAccessor, allBindings) {
                ...
                return $(element).daterangepicker(options, function(
                    startDate,
                    endDate,
                    period
                ) {
            ...
            this.prevDate = ko.pureComputed(
                (function(_this) {
                    return function() {
                        ...
                        (ref = _this.period.nextPageArguments()), (amount =
                            ref[0]), (period = ref[1]);
    ...
    CalendarView = (function() {
        function CalendarView(mainView, dateSubscribable, type) {
            ...
            this.inputDate = ko.computed({
                read: (function(_this) {
                    return function() {
                        return (_this.hoverDate() || _this.activeDate())
                            .format(_this.locale.inputFormat);
                    };
                })(this),
                write: (function(_this) {
                    return function(newValue) {
        ...
        CalendarView.prototype.cssForDate = function(date, periodIsDay) {
            ...
            }), (obj1[this.type + "-date"] = onRangeEnd), (obj1[
                "clickable"
                ] = withinBoundaries), (obj1[
                "out-of-boundaries"
                ] = !withinBoundaries), (obj1["unavailable"] =
                periodIsDay && differentMonth), obj1;
        };
